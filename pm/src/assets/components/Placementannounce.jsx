...
function Placementsannounce() {
  ...
  const fetchCompanies = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/companies/get-companies`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${API_BASE_URL}/api/companies/addcompany`,
        formDataToSend
      );
  ...
  const fetchCompaniess = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/companies/get-companies`);
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`${API_BASE_URL}/api/companies/deletecompany/${id}`);
          fetchCompaniess();
          window.location.reload()
            ...
            <div className="companies-grid" style={{ position: 'relative', top: "-30px", left: '30px', marginBottom: '200px', marginTop: '40px' }}>
              {companies.map((company) => (
                <div key={company.ID} className="company-card" data-bs-toggle="offcanvas" data-bs-target="#companyDetails" onClick={() => openCompanyDetails(company)}>
                  <img src={`${API_BASE_URL}/${company.COMPANYIMG}`} alt={company.COMPANYNAME} className="company-logo" />
                      ...
                      <tbody>
                        {displayedData.map((student, index) => (
                          <tr key={student._id}>
                            ...
                            <td>
                              <img
                                src={student.profileImage ? `${API_BASE_URL}/${student.profileImage}` : "/default-avatar.png"}
                              alt="Profile"
