...
function Trainingattendance() {
  ...
  const fetchBatchAttendance = async (scheduleCodee, batchNumber) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/attendance/attendance/${scheduleCodee}/${batchNumber}`);
  ...
  const handleBatchClick = async (batch, scheduleCode, trainingName) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/attendancestudent/attendance-students`, {
  ...
  const fetchAvailableDates = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/schedule/schedule-dates?scheduleCode=${selectedScheduleCode}&batch=${selectedBatch}`
      );
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/attendance/save`,
        attendanceData
      );
                    ...
                    {students.map((student) => {
                      ...
                      return (
                        <tr key={student.registration_number}>
                          ...
                          <td>
                            <button
                              className={`btn ${selectedAction === "P"
                                ? "btn-success"
                                : selectedAction === "A"
                                  ? "btn-danger"
                                  : "btn-warning"
